...
recevoirJetDe(resultat){
    ...
    // =========================
    // SEUIL DE BASE + BONUS ÉQUIPEMENT
    // =========================

    const bonusSeuil =
        this.lanceur.getBonusSeuil();

    let seuilJet =
        competence.seuil + bonusSeuil;

    if(bonusSeuil !== 0){

        ajouterAuJournal(
            `🎯 ${this.lanceur.nom} gagne +${bonusSeuil} au seuil grâce à son équipement.`
        );
    ...
    if(deconcentration){
        ...
        ajouterAuJournal(
            `${this.lanceur.nom} est déconcentré : seuil ${competence.seuil} + ${bonusSeuil} - ${deconcentration.valeur} = ${seuilJet}.`
        );
...
resoudreSoins(){
    ...
    this.ciblesSelectionnees.forEach(cible => {
        ...
        const bonusSoins =
        this.lanceur.getBonusSoins();

        const soinBase =
        competence.soin + bonusSoins;

        if(bonusSoins !== 0){

            ajouterAuJournal(
                `💚 ${this.lanceur.nom} gagne +${bonusSoins} aux soins grâce à son équipement.`
            );

        }

        const soin =
        soinBase * multiplicateur;
...
resoudreDegats(){
    ...
    // =====================================================
    // ⚔️ DÉGÂTS DE BASE + BONUS D'ÉQUIPEMENT
    // =====================================================

    const degatsBase =
        competence.degats + this.lanceur.getDegats();
    ...
    this.ciblesSelectionnees.forEach(cible => {
        ...
        let seuilDefense =
            this.seuilFinal - cible.getDefense();

        ajouterAuJournal(
            `${cible.nom} : seuil ${this.seuilFinal} - défense ${cible.getDefense()} = ${seuilDefense}.`
        );
